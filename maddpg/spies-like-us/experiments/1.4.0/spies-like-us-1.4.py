"""
A scenario in which there are two kinds of agents, Spies and Plebs.

Spies work counter to the goal(s) of Plebs.
Spies appear on the outside to be no different from Plebs.
All agents can recognize each other by "name".

To allow spies to be effective, we will build a cooperative game that
Plebs must play which has an opportunity to intervene and subvert.

Games:
  Take the food to the nest.
  Move the block.
"""
